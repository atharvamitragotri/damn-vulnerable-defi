...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        /** Create and deploy a contract where we execute a flashloan for the DVT balance of the Pool.
            Using this balance, we take a snapshot and use it to acquire the majority votes in the governance
            contract. Now all we have to do is return the flashloan, queue the attack with a call to drainAllFunds
            with the attacker address, and wait for the required delay time(2 days), then execute the queued action.
         */
        ...
        await this.selfieattack.planAttack();
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 5 days
        await this.selfieattack.executeAttack();
    });
